...
const tests = [
  ...
  { file: 'nested/really-nested/fullPath' },
  { file: 'nested/really-nested/longerPath', options: { maxDepth: 2 } },
  { file: 'nested/really-nested/shortPath', options: { maxDepth: 1 } },
  { file: 'nested/really-nested/maxDepthLongerThanNecessary', options: { maxDepth: 6 } },
  ...
  describe('No preset', function () {
    tests.forEach(function (test) {
      ;(test.only ? it.only : it)(`${test.file}`, function (done) {
        const transform = transformFileSync(path.join('test/src/', `${test.file}.js`), {
